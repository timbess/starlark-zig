        ...
        .number_literal => {
            ...
            return switch (number_res) {
                ...
                .float => {
                    ...
                    return &(try Runtime.StarFloat.init(n)).obj;
                },
            };
        },
        .string => {
            // Use stack for most string literals.
            var stack_alloc = std.heap.stackFallback(knobs.string_avg_len, Gc.allocator());
            const tmp_alloc = stack_alloc.get();
            const string_literal = source[token.loc.start..token.loc.end];
            const str_parsed = std.zig.string_literal.parseAlloc(tmp_alloc, string_literal) catch return error.LiteralInvalid;
            defer tmp_alloc.free(str_parsed);
...
pub fn compile(fallback: Allocator, module_name: []const u8, source: [:0]const u8, ast: *const Ast, comptime opts: Opts) Error!Module {
    ...
    var interner = try StringInterner.init(Gc.allocator());
    defer interner.deinit();

    return try compileModule(gpa, module_name, source, ast, &interner);
}
